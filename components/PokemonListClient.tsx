    ...
    try {
      if (activeGen || activeType) {
        ...
        if (nextSlice.length > 0) {
          ...
          if (nextIdx + pageSize >= filteredMasterList.length) {
            setHasMore(false);
          }
        } else {
          setHasMore(false);
  ...
  return (
    <div className="pb-10 relative">
      ...
      <div className="mb-6 -mt-4 relative z-10">
        <div className="bg-white/80 backdrop-blur-md border border-white/60 rounded-3xl shadow-lg shadow-blue-900/5 overflow-hidden transition-all duration-500 ease-in-out">
            ...
            <div className="flex items-center gap-4 overflow-hidden">
              ...
              {!isFilterOpen && (
                <div className="flex gap-2 items-center overflow-x-auto no-scrollbar pl-2">
                  {activeGen && (
                    <span className="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-xs font-bold whitespace-nowrap border border-blue-200">
                      {activeGenName}
                    </span>
                  )}
                  {activeType && (
                    <span className={`px-3 py-1 rounded-full text-xs font-bold whitespace-nowrap border text-white ${TYPE_COLORS[activeType]}`}>
                      {activeType}
                    </span>
                  )}
                  {!activeGen && !activeType && (
                    <span className="text-xs text-gray-400 font-medium italic">All Pokemon</span>
                  )}
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-3 shrink-0">
               {(activeGen || activeType) && (
                <button 
                  ...
                  className="text-xs font-bold text-red-500 hover:text-red-700 px-3 py-1.5 rounded-full hover:bg-red-50 transition-colors"
                >
                  Reset
                </button>
              )}
              <div className={`p-2 rounded-full bg-gray-50 text-gray-400 transition-transform duration-300 ${isFilterOpen ? "rotate-180 bg-gray-100 text-gray-600" : ""}`}>
          ...
          <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isFilterOpen ? "max-h-[1000px] opacity-100" : "max-h-0 opacity-0"}`}>
            <div className="p-6 pt-0 border-t border-gray-100">
              ...
              <div className="mb-6 mt-4">
                <h3 className="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-3">Region / Generation</h3>
                <div className="flex flex-wrap gap-2">
                  <button
                    onClick={() => handleFilterChange("", activeType)}
                  ...
                  {GENERATION_DATA.map((gen) => (
                    <button
                      key={gen.id}
                      onClick={() => handleFilterChange(gen.id, activeType)}
                      className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border ${
                        activeGen === gen.id 
              ...
              <div>
                <h3 className="text-[10px] font-extrabold text-gray-400 uppercase tracking-widest mb-3">Pokemon Type</h3>
                <div className="flex flex-wrap gap-2">
                  <button
                    onClick={() => handleFilterChange(activeGen, "")}
                    className={`px-4 py-2 rounded-xl text-xs font-bold transition-all border ${
                      activeType === "" 
                        ? "bg-gray-800 text-white border-gray-800 shadow-lg shadow-gray-200" 
                        : "bg-white text-gray-500 border-gray-100 hover:border-gray-300"
                  ...
                  {Object.keys(TYPE_COLORS).map((type) => (
                    type !== 'unknown' && type !== 'shadow' && (
                      <button
                        key={type}
                        onClick={() => handleFilterChange(activeGen, activeType === type ? "" : type)}
                        className={`px-4 py-2 rounded-xl text-xs font-bold transition-all capitalize border ${
                          activeType === type 
                            ? `${TYPE_COLORS[type]} text-white border-transparent shadow-md` 
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-12 animate-fadeIn">
          {pokemons.map((pokemon) => (
            <Link
              ...
              className="group relative bg-white rounded-[2rem] p-5 shadow-sm hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-300 hover:-translate-y-2 block"
            >
              <div className="absolute inset-0 bg-gradient-to-br from-gray-50/50 to-white rounded-[2rem] -z-10" />
              <span className="absolute top-4 right-6 text-5xl font-extrabold text-gray-100/80 z-0 group-hover:text-gray-200/50 transition-colors">
                #{String(pokemon.id).padStart(3, "0")}
              </span>

              <div className="relative z-10 flex flex-col items-center">
                <div className="w-32 h-32 mb-5 relative transition-transform duration-300 group-hover:scale-110 drop-shadow-sm group-hover:drop-shadow-lg">
                   <Image 
                      src={pokemon.image} 
                      alt={pokemon.name}
                      fill
                ...
                <h2 className="text-lg font-extrabold text-gray-800 capitalize tracking-tight mb-2 group-hover:text-blue-600 transition-colors">
                ...
                <div className="flex gap-1.5 justify-center">
                  {pokemon.types.map((type) => (
                    <span 
                      key={type} 
                      className={`${TYPE_COLORS[type] || "bg-gray-400"} text-white text-[10px] font-bold px-3 py-1 rounded-full capitalize shadow-sm`}
                    >
                      {type}
                    </span>
                  ))}
      ...
      {hasMore && (
        <div className="flex justify-center">
          <button
            onClick={loadMore}
            disabled={loading}
            className="px-8 py-3 bg-white border border-gray-100 text-gray-800 rounded-full font-bold shadow-lg shadow-gray-200/50 hover:bg-gray-50 hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 group"
          >
            {loading ? (
              <>
              ...
              <>
                <span>Load More</span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-gray-400 group-hover:translate-y-0.5 transition-transform"><path d="m6 9 6 6 6-6"/></svg>
              </>
            )}
          </button>
        </div>
      ...
      <button
        onClick={scrollToTop}
        className={`fixed bottom-8 right-8 z-50 p-4 rounded-full bg-gray-900 text-white shadow-2xl transition-all duration-500 ease-in-out transform hover:bg-black hover:scale-110 active:scale-95 ${showBackToTop ? "translate-y-0 opacity-100" : "translate-y-20 opacity-0 pointer-events-none"}`}
      >
