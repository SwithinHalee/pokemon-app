...
export default function PokemonDetailClient({ pokemon }: Props) {
  ...
  const badgeColor = TYPE_COLORS[mainType] || "bg-gray-400";
  ...
  // Pilih Gambar berdasarkan State Shiny 
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${bgGradient} flex items-center justify-center p-4 md:p-8`}>
      <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-6xl flex flex-col md:flex-row overflow-hidden min-h-[850px] relative">
        ...
        <div className="md:w-1/2 p-8 md:p-12 flex flex-col relative z-10">
          ...
          <div className="flex-1 flex items-center justify-center py-4 relative">
             <div className="relative w-64 h-64 md:w-80 md:h-80 hover:scale-105 transition-transform duration-500 z-20 cursor-pointer">
                {/* Key berubah agar animasi refresh saat ganti shiny */}
                <PokemonImage
                  key={currentImage} 
                  src={currentImage}
                  alt={pokemon.name}
        ...
        <div className="md:w-1/2 p-8 md:p-12 bg-white flex flex-col">
          ...
          <div className="flex gap-8 border-b border-gray-100 mb-6 pb-1">
          ...
          {activeTab === "About" && (
            <div className="animate-fadeIn h-full overflow-y-auto pr-2 custom-scrollbar">
              ...
              {/* Versions Toggle (Sekarang Berfungsi!)  */}
              <div className="mb-6">
              ...
              <div className="grid grid-cols-3 gap-3 mb-8">
                {/* ... Height, Weight, Category, Gender (Sama seperti sebelumnya) ... */}
              ...
              {/* Game Appearances (Fitur Baru)  */}
              <div className="mb-8">
                 <h3 className="font-bold text-gray-900 text-sm mb-3">Game Appearances</h3>
                 <div className="flex gap-2 flex-wrap h-24 overflow-y-auto custom-scrollbar pr-2">
          ...
          {activeTab === "Moves" && (
            <div className="animate-fadeIn h-full overflow-y-auto pr-2 custom-scrollbar">
              <div className="flex flex-wrap gap-2">
                {moves.map((m) => (<div key={m.move.name} className="bg-gray-50 px-3 py-1.5 rounded-lg text-xs text-gray-700 font-bold capitalize border border-gray-200 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition-colors">{m.move.name.replace("-", " ")}</div>))}
              </div>
            </div>
          )}
