...
export default function PokemonDetailClient({ pokemon, prevPokemon, nextPokemon }: Props) {
  ...
  const availableGenerations = useMemo(() => {
    ...
    pokemon.moves.forEach((move) => {
      move.learn_details.forEach((detail) => gens.add(detail.generation));
    });
  ...
  const filteredMoves = useMemo(() => {
    ...
    return pokemon.moves
      .flatMap((move) => {
        const relevantDetails = move.learn_details.filter((detail) => detail.generation === selectedGen && (selectedMethod === "all" || detail.method === selectedMethod));
  ...
  }, [pokemon.moves, selectedGen, selectedMethod]);
      ...
      <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-[85rem] flex flex-col md:flex-row overflow-hidden h-[850px] relative">
        ...
        <div className="md:w-[45%] py-12 pr-10 pl-10 md:pl-52 flex flex-col relative h-full justify-between border-r border-gray-50 overflow-hidden">
          <div className="w-full mb-4 relative z-10">
            ...
            <h1 className="text-4xl lg:text-5xl font-extrabold capitalize text-gray-800 tracking-tight leading-tight break-words" title={pokemon.name}>
          ...
          <div className="flex-1 overflow-hidden">
              ...
              <div className="h-full flex flex-col">
                <div className="flex flex-wrap gap-4 mb-6 shrink-0 p-1">
                  <div className="flex flex-col gap-1 relative">
                    <label htmlFor="gen-select" className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">
                      Game Version
                    </label>
                  ...
                  <div className="flex flex-col gap-1 relative">
                    <label htmlFor="method-select" className="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">
                      Learn Method
                    </label>
                    <div className="relative group">
                      ...
                      <tbody>
                        {filteredMoves.map((move, index) => (
                          <tr key={`${move.name}-${index}`} className="hover:bg-gray-50 transition-colors border-b border-gray-50 last:border-none">
                            ...
                            <td className="p-2 font-bold text-gray-700 text-sm text-center">{move.power || "—"}</td>
                            <td className="p-2 font-bold text-gray-700 text-sm text-center">{move.accuracy ? `${move.accuracy}%` : "—"}</td>
                            <td className="p-2 font-bold text-gray-700 text-sm text-center">{move.pp}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            {(activeTab === "Episodes" || activeTab === "Cards") && (
              <div className="h-full flex flex-col items-center justify-center text-gray-300">
                <p className="font-bold text-lg italic">Coming Soon</p>
              </div>
            )}
          </div>
        </div>
